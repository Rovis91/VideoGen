...
async function chatCompletion(apiKey, messages, options = {}, signal) {
  ...
  if (content == null || content === '') {
    ...
    const debug = !hint && data ? ` Response keys: ${Object.keys(data).join(', ')}.` : '';
    throw new Error(hint ? `No content in chat response. ${hint}` : `No content in chat response.${debug} Try again or use a different prompt.`);
  }
  return content;
}
